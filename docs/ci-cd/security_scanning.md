...
OPEN_ALERTS=$(gh api repos/OWNER/REPO/dependabot/alerts \
...
if [ "$OPEN_ALERTS" -gt 0 ]; then
  ...
  echo "Run: gh api repos/OWNER/REPO/dependabot/alerts --jq '.[] | select(.state == \"open\")' for details"
...
```json
{
  "scripts": {
    ...
    "security:fix": "npm audit fix",
    "security:check:github": "gh api repos/OWNER/REPO/dependabot/alerts --jq '.[] | select(.state == \"open\")'",
